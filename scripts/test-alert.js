...
if (!process.env.TEST_API_KEY) {
  console.warn(
    "Warning: TEST_API_KEY not found in environment variables. Please set it in your .env file."
  )
}
  ...
  try {
    ...
    setTimeout(async () => {
      ...
      if (!upResponse.ok) {
        ...
        throw new Error(
          `HTTP error! status: ${upResponse.status}, message: ${error}`
        )
      }
  ...
  try {
    ...
    setTimeout(async () => {
      ...
      if (!recoveryResponse.ok) {
        ...
        throw new Error(
          `HTTP error! status: ${recoveryResponse.status}, message: ${error}`
        )
      }
