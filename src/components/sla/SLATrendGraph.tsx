...
export function SLATrendGraph({ 
  ...
  const { data, stats } = useMemo(() => {
    ...
    const processedData = filteredData.map((m) => ({
    ...
    const stats = {
      ...
      average: processedData.length ? 
        Number((processedData.reduce((acc, curr) => acc + curr.uptime, 0) / processedData.length).toFixed(2)) : 0,
      min: processedData.length ? Math.min(...processedData.map(d => d.uptime)) : 0,
      max: processedData.length ? Math.max(...processedData.map(d => d.uptime)) : 0
    };
  ...
  if (!data.length) {
    return (
      <Card className="col-span-full">
        <CardContent className="flex items-center justify-center p-6">
          <p role="status">No SLA data available for the selected time range.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="col-span-full hover:shadow-lg transition-shadow duration-200">
      ...
      <CardContent>
        <div className="h-[300px] w-full mt-4" role="img" aria-label="SLA Trend Graph">
