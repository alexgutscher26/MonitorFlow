...
export async function POST(req: Request) {
  try {
    ...
    if (user.SLADefinitions.length >= maxSLAs) {
      return new NextResponse(`You have reached the maximum number of SLAs (${maxSLAs}) for your plan. Please upgrade to create more.`, { status: 403 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!slaId) {
      return NextResponse.json({ message: "SLA ID is required" }, { status: 400 });
    }
