 ...
 *
 * Displays a message prompting the user to create their first event for a specified category.
 * Includes a code snippet for API usage, and periodically checks if events exist for the category.
 *
 * @param {EmptyCategoryStateProps} props - The props object containing the category name.
 * @returns {JSX.Element} The empty state display with an API code example.
 */
export const EmptyCategoryState = ({ categoryName }: EmptyCategoryStateProps) => {
  const router = useRouter();

  // Query to poll if the category has events
  const { data } = useQuery({
  ...
  // Refresh the page if events are detected
  useEffect(() => {
    if (hasEvents) {
      router.refresh();
    }
  ...
  // Example code snippet for the API request
  ...
  return (
    ...
    >
      <h2 className="text-xl font-medium text-center tracking-tight text-gray-950">
