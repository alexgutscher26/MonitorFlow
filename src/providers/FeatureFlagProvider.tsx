...
const featureFlagSchema = z.object({
  ...
  conditions: z.array(z.object({
    ...
    operator: z.enum(["equals", "notEquals", "contains", "notContains", "before", "after"]),
    value: z.any(),
  })).optional(),
...
export function FeatureFlagProvider({
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await fetch(`/api/feature-flags?environment=${environment}`);
        if (!response.ok) {
  ...
  const checkConditions = useCallback((flag: FeatureFlag): boolean => {
    ...
    return flag.conditions.every(condition => {
      switch (condition.type) {
        ...
        case "group":
          ...
          const userGroups = user?.organizationMemberships?.map(m => m.organization.id) || [];
        ...
        case "time": {
          const now = new Date();
          const [hours, minutes] = (condition.value as string).split(":").map(Number);
          const time = new Date();
  ...
  const checkGradualRollout = useCallback((flag: FeatureFlag): boolean => {
    if (!flag.rolloutConfig?.startDate) return true;

    const now = new Date();
    const start = new Date(flag.rolloutConfig.startDate);
    const end = flag.rolloutConfig.endDate ? new Date(flag.rolloutConfig.endDate) : null;
  ...
  const checkFlag = useCallback((key: string): boolean => {
    ...
    switch (flag.type) {
      ...
      case "schedule":
        const now = new Date();
        const start = flag.value?.startDate ? new Date(flag.value.startDate) : null;
  ...
  const checkFlagValue = useCallback(<T,>(key: string, defaultValue: T): T => {
    const flag = flags.find((f) => f.key === key);
    if (!flag || !checkFlag(key)) return defaultValue;
    return (flag.value as T) ?? defaultValue;
  }, [flags, checkFlag]);

  const getFlagDetails = useCallback((key: string): FeatureFlag | null => {
  ...
  const contextValue = useMemo(() => ({
    ...
    isEnabled,
    refreshFlags,
  }), [flags, isLoading, error, checkFlag, checkFlagValue, getFlagDetails, isEnabled, refreshFlags]);
